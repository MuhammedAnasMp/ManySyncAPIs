...
def upload_reel_task(reel_upload_id):
    ...
    client = Client()
    client.login(reel_upload.account.username,
                 reel_upload.account.password)  # Use proper login
        ...
        try:
            ...
            instagram_account, created = InstagramAccount.objects.update_or_create(
                ...
                defaults={
                    ...
                    "followers_count": public_info.follower_count,
                    "following_count": public_info.following_count,
                    "total_posts": public_info.media_count,
                },
            )
            ...
            if challenge_info.get("step_name") in ["select_method", "close"]:
                return JsonResponse(
                    {"error": "Verification required. Check Instagram for verification."}, status=404
                )
...
class IgDeleteeClassView(APIView):
    def delete(self, request, *args, **kwargs):
        ...
        instagram_account = get_object_or_404(
            InstagramAccount, id=account_id, user=request.user)
...
class DashboardData(APIView):
    def get(self, request, igAccountId):
        ...
        print("str(instagram_account.ig_user_id):",
              str(instagram_account.ig_user_id))
        ...
        try:
            ...
            print("igAccountId", igAccountId)
            print("user=request.user", request.user)
            instagram_account = InstagramAccount.objects.get(
                id=igAccountId, user=request.user)
            data = {
                "username": instagram_account.username,
                # High-resolution profile pic
                "profile_pic_url": str(instagram_account.profile_pic_url),
                "followers_count": instagram_account.follower_count,
                "following_count": instagram_account.following_count,
                "total_posts": instagram_account.media_count,
                "is_private": instagram_account.is_private,
                "biography": instagram_account.biography,
                "is_verified": instagram_account.is_verified,
                "external_url": instagram_account.external_url,  # Website if available
            }
